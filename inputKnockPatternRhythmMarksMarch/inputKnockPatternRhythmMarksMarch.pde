...
ArrayList<Tally> tallycount = new ArrayList<Tally>();//little dots at the top that keep track of whats going on (idk if I have this cleared...might be an issue)
...
FloatList allIntervalRatios;//store all interval ratios and eventually calculate Standard Deviation...i believe it is each row for one iteration of the rhythm
...
void calculateRhythm(){
  ...
  println("intervals in millis");
   for(int i=0; i<knockIntervals.size();i++){float interval = knockIntervals.get(i);println("interval "+(i+1)+": "+interval);
  }
  //calculate interval ratios
  for(int i=1; i<knockIntervals.size();i++){
    float intervalRelation = knockIntervals.get(i-1)/knockIntervals.get(i);
    allIntervalRatios.append(intervalRelation);
    println("intervalratios "+(i)+": "+intervalRelation);}
  
  //set all the current tally marks to fade away
  for(int i=0; i<tallycount.size();i++){
    tallycount.get(i).currentKnockIteration = false;
  }
}
void exportRhythm(){//calculate standard deviation across several inputs - this is the average of the squared differences each ratio has to the average of the raitos
  ...
  JSONArray averageIntervalRatio = new JSONArray();
  for(int col=0; col<knockIntervals.size()-1; col++){
    float average=0;//find the average interval ratio from the list of ratios
